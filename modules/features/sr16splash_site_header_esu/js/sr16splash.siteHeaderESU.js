(function ($) {
  ...
  /**
   * Behaviour for the sr16splash Site ESU Header
   *
   * Author: J.Pitt
   * Contributors: -
   * 
   ...
   * 
   */
  Drupal.behaviors.sr16splashsiteHeaderESU = {
    ...
    attach: function (context, settings) {
      var _base = Drupal.behaviors.sr16splashsiteHeaderESU;

      // Setup the email signup, passing in the relevant context
      $(_base.defaults.siteHeaderESUClass, context).once('sr16splashsiteHeaderESU', _base.setupESU);

      // If we do not have a ESU Header, the settings won't be there so just skip this behavior
      var _settings = Drupal.settings.sr16splashsiteHeaderESU;
      if (!_settings) return;
    ...
    setupESU : function(context) {
      ...
      Drupal.settings.sr16splashsiteHeaderESU = $.extend({}, _base.defaults);
    ...
    EH_toggleESU : function(e, eventParam) {
      var _base = Drupal.behaviors.sr16splashsiteHeaderESU;
      var _menuBase = Drupal.behaviors.sr16splashtouchMenu;
      var _settings = Drupal.settings.sr16splashsiteHeaderESU;
      e.preventDefault();
