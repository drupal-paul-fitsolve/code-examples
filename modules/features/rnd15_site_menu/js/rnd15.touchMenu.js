(function ($) {
   ...
   * -- CRL2 Breakpoints
   */
  Drupal.settings.rnd15touchMenu = Drupal.settings.rnd15touchMenu || {};

  Drupal.behaviors.rnd15touchMenu = {
    ...
    attach: function (context, settings) {
      var _base = Drupal.behaviors.rnd15touchMenu;

      // Setup the menu, passing in the relevant context
      $(_base.defaults.mainNavClass, context).once('rnd15touchMenu', function(){
        // Extend the current behaviour with default values
        Drupal.settings.rnd15touchMenu = $.extend({}, Drupal.settings.rnd15touchMenu, _base.defaults);
    ...
    setupTouchMenu : function(context) {
      var _base = Drupal.behaviors.rnd15touchMenu;
      var _settings = Drupal.settings.rnd15touchMenu;
    ...
    checkDebugMode: function(){
      ...
      if(toggleDebug && typeof $.cookie !== 'undefined') {
        $.cookie('rnd15touchMenuDebug', toggleDebug, { expires: 1, path: '/'});
        if(toggleDebug === 'false') {
          // Deletes the cookie
          $.cookie('rnd15touchMenuDebug', null, { expires: 1, path: '/' });
        }
      }
      return (typeof $.cookie !== 'undefined') ? ($.cookie('rnd15touchMenuDebug') === 'true') : false;
    },
     ...
     * Drupal.behaviors.rnd15touchMenu.closeNavButtons.apply($(navButton));
     */
  ...
  Drupal.behaviors.rnd15touchMenu.fastTouchDetect();
